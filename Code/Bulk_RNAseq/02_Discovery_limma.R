...
setwd("./A04_20230601_Discovery_limma/")
if(dir.exists("./temp")) {
...
pc_discovery_all <- ggplot(pc_df, aes(PC1, PC2, fill = group)) +
...
pc_discovery_split <- ggplot(pc_df, aes(PC1, PC2, fill = group)) +
  geom_hline(yintercept = 0, lty = 2) +
  geom_vline(xintercept = 0, lty = 2) +
  # geom_hdr(probs = c(0.5), alpha = 0.5) +
  geom_point(size = 2, shape = 21, colour = "black", alpha = 0.8) +
...
corfit <- duplicateCorrelation(v,
                               design,
                               block = norm_data$samples$patient)
corfit$consensus
# [1] 0.2334668
...
results_treat$high_vs_low_risk_Bod %>%
...
results_treat$high_vs_low_risk_Ant %>%
  mutate(Sign = ifelse(logFC>0 & adj.P.Val<=0.05, "Up", ifelse(logFC<0 & adj.P.Val<=0.05, "Down", "Non-DEG"))) %>%
  mutate(trans = ifelse(Sign =="Non-DEG", 0.2, 0.8)) %>%
  ggplot(aes(AveExpr, logFC, col = Sign, label = label, alpha = trans)) +
  ggtitle("Antrum") +
...
Heatmap(t(scale(t(cpm(norm_data[degs,], log = T)))),
        name = "Z-score",
        col = col_fun,
        column_split = 3,
...
save(discovery_ebayes = results_eBayes,
     ...
     file = "DE_results_discovery.rda")

saveRDS(norm_data, "Discovery_norm_data.rds")
